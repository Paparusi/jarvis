...
class TestToolResultCache:
    ...
    async def test_first_call_executes_tool(self, agent_loop):
        result = await agent_loop._execute_tool_cached("web_search", {"query": "test"})
    ...
    async def test_second_call_uses_cache(self, agent_loop):
        await agent_loop._execute_tool_cached("web_search", {"query": "test"})
        await agent_loop._execute_tool_cached("web_search", {"query": "test"})
    ...
    async def test_different_args_not_cached(self, agent_loop):
        await agent_loop._execute_tool_cached("web_search", {"query": "test1"})
        await agent_loop._execute_tool_cached("web_search", {"query": "test2"})
        assert agent_loop._tools.execute.call_count == 2

    @pytest.mark.asyncio
    async def test_different_tools_not_cached(self, agent_loop):
        await agent_loop._execute_tool_cached("web_search", {"query": "test"})
        await agent_loop._execute_tool_cached("fetch_url", {"query": "test"})
        assert agent_loop._tools.execute.call_count == 2
    ...
    async def test_cache_returns_same_result(self, agent_loop):
        result1 = await agent_loop._execute_tool_cached("web_search", {"query": "test"})
        ...
        result2 = await agent_loop._execute_tool_cached("web_search", {"query": "test"})
